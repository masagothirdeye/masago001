...
if c_step == 1:
    ...
    # 選択肢を無くし、自由に変形できる案内のみにシンプル化
    st.markdown("""
    <div class="notice-box">
        <h4 style='color: #e65100; margin-top:0; font-size:16px;'>📐 写真のサイズ調整について</h4>
        <p style='margin-size: 13px; margin-bottom: 0;'>写真の上の<b>「赤い枠」</b>を指で動かすことで、お好みの大きさに自由にカットできます！</p>
    </div>
    """, unsafe_allow_html=True)
    
    uploaded_file = st.file_uploader("スマホやPCから画像を選択（JPG / PNG）", type=["jpg", "jpeg", "png"])
    
    # もし過去にアップロードした画像があれば、ファイル選択が空でもそれを読み込む（画像消失対策）
    if uploaded_file:
    ...
    if raw_img:
        ...
        st.info("💡 スマホの方は、写真の上の【赤い枠の四隅】を触って広げたり、中を触って移動させてください。")
        
        # 🎨 box_color='#FF0000' で赤い枠線に変更 / aspect_ratio=None で自由サイズ
